					...
					if(isset($args[1])){
						if(isset($this->firstPosition[$playerName], $this->secondPosition[$playerName])){
							if(!isset($this->areas[strtolower($args[1])])){
								new Area(strtolower($args[1]), ["edit" => true, "god" => false, "touch" => true, "combo" => false, "nofalldamage" => false], $this->firstPosition[$playerName], $this->secondPosition[$playerName], $sender->getLevel()->getName(), [$playerName], $this);
								$this->saveAreas();
						...
						if(isset($this->areas[strtolower($args[1])])){
							...
							if(isset($args[2])){
								if(isset($area->flags[strtolower($args[2])])){
									...
									$o = TextFormat::RED . "Flag not found. (Flags: edit, god, touch, combo, nofalldamage)";
								}
							}else{
								$o = TextFormat::RED . "Please specify a flag. (Flags: edit, god, touch, combo, nofalldamage)";
							}
						}else{
